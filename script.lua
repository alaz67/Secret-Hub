-- Secret Hub - Pure Black & White Edition
-- Discord: .gg/secrethub
...
do
    ...
    if ok and built then
        ...
        warn("[SecretHub] raw synchronizer init failed:", built)
        ZawaRawSync = { caches = {} }
    end
end
...
local FileName = "SecretHub_v1.json" 
local PriorityFileName = "SecretHub_priority.json"
local DefaultConfig = {
    Positions = {
...
local function applyTextureBg(parent)
    -- Prüfen ob parent ein gültiges Objekt für ImageLabel ist
    if not parent or not parent:IsA("Frame") and not parent:IsA("TextButton") and not parent:IsA("TextBox") and not parent:IsA("ScrollingFrame") and not parent:IsA("CanvasGroup") and not parent:IsA("ViewportFrame") then
        return nil
    end
...
local function AddResizeHandle(frame, panelKey, posOverride)
    ...
    -- Resize Handle Hintergrund
    local handleBg = applyTextureBg(handle)
    if handleBg then handleBg.ZIndex = 0 end

    local hStroke = Instance.new("UIStroke", handle)
    hStroke.Color = Color3.fromRGB(100, 100, 100)
    hStroke.Thickness = 1
...
local function AddMobileMinimize(frame, labelText)
    ...
    -- Minimize Button Hintergrund
    local minBg = applyTextureBg(minimizeBtn)
    if minBg then minBg.ZIndex = 0 end
    ...
    -- Restore Button Hintergrund
    local resBg = applyTextureBg(restoreBtn)
    if resBg then resBg.ZIndex = 0 end
...
local function ShowNotification(title, text)
    ...
    -- Notification Hintergrund
    local notBg = applyTextureBg(f)
    if notBg then notBg.ZIndex = 0 end
...
do
    ...
    function setFPSBoost(enabled)  
        ...
        if enabled then
            ...
            print("[SecretHub] Optimizer Loaded ✓")
        end
...
local function startAntiRagdoll(mode)
    ...
    local _arThread = task.spawn(function()
        while true do
            task.wait(0.03)
            if not (Connections.antiRagdollConn ~= false) then break end
    ...
    Connections.antiRagdollConn = {Disconnect = function() task.cancel(_arThread); Connections.antiRagdollConn = false end}
end
...
local function hookAntiRagV2(char)
    ...
    local _c4Active = true
    local c4Thread = task.spawn(function()
        while _c4Active do
            task.wait(0.05)
            local st = hum:GetState()
            if st == Enum.HumanoidStateType.Physics or st == Enum.HumanoidStateType.Ragdoll
                ...
                if (vel - lastVel).Magnitude > 40 and vel.Magnitude > 25 then
                    hrp.Velocity = vel.Unit * math.min(vel.Magnitude, 15)
                end
            end
            lastVel = hrp.Velocity
        end
    end)
    local c4 = {Disconnect = function() _c4Active = false; task.cancel(c4Thread) end}
...
task.spawn(function()
    ...
    frame.Size = UDim2.new(0, 280*mobileScale, 0, 540*mobileScale)
    ...
    -- AutoSteal Frame Hintergrund
    local asBg = applyTextureBg(frame)
    if asBg then asBg.ZIndex = 0 end
    ...
    AddMobileMinimize(frame, "SECRET HUB")
    ...
    -- Header Hintergrund
    local hdrBg = applyTextureBg(header)
    if hdrBg then hdrBg.ZIndex = 0 end
    ...
    logoCircle.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
    ...
    stealRefreshBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    stealRefreshBtn.Text = "REFRESH"
    stealRefreshBtn.Font = Enum.Font.GothamBold
    stealRefreshBtn.TextSize = 11
    stealRefreshBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    ...
    -- Refresh Button Hintergrund
    local refBg = applyTextureBg(stealRefreshBtn)
    if refBg then refBg.ZIndex = 0 end
    ...
    stealRefreshBtn.MouseEnter:Connect(function()
        stealRefreshBtn.BackgroundColor3 = Color3.fromRGB(55, 55, 55)
    ...
    if IS_MOBILE then
        ...
        menuToggleBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
        menuToggleBtn.Text = "MENU"
        menuToggleBtn.Font = Enum.Font.GothamBold
        menuToggleBtn.TextSize = 10
        menuToggleBtn.TextColor3 = Color3.fromRGB(220, 220, 220)
    ...
    -- Status Row Hintergrund
    local srBg = applyTextureBg(statusRow)
    if srBg then srBg.ZIndex = 0 end

    local statusDot = Instance.new("Frame", statusRow)
    statusDot.Size = UDim2.new(0, 7, 0, 7)
    statusDot.Position = UDim2.new(0, 10, 0.35, 0)
    statusDot.AnchorPoint = Vector2.new(0, 0.5)
    statusDot.BackgroundColor3 = Color3.fromRGB(130, 130, 130)
    ...
    stealStatusLbl.TextColor3 = Color3.fromRGB(180, 180, 180)
    ...
    targetsLbl.TextColor3 = Color3.fromRGB(150, 150, 150)
    ...
    playersLbl.TextColor3 = Color3.fromRGB(150, 150, 150)
    ...
    selectLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
    ...
    -- List Frame Hintergrund
    local lfBg = applyTextureBg(listFrame)
    if lfBg then lfBg.ZIndex = 0 end
    ...
    -- Toggle Button Container Hintergrund
    local tbBg = applyTextureBg(toggleBtnContainer)
    if tbBg then tbBg.ZIndex = 0 end
    ...
    enableBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
    enableBtn.BorderSizePixel = 0
    enableBtn.AutoButtonColor = false
    Instance.new("UICorner", enableBtn).CornerRadius = UDim.new(0, 10)

    -- Enable Button Hintergrund
    local enBg = applyTextureBg(enableBtn)
    if enBg then enBg.ZIndex = 0 end
    ...
    customizePriorityBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
    ...
    customizePriorityBtn.TextColor3 = Color3.fromRGB(170, 170, 170)
    ...
    local function makeModeRow(parent, labelText, yPos, isOn, onToggle)
        ...
        if pillBg then pillBg.ZIndex = 0 end

        local function refresh(on)
            pill.Text = on and "ON" or "OFF"
            pill.BackgroundColor3 = on and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(30, 30, 30)
            pill.TextColor3 = on and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(180, 180, 180)
        end
        refresh(isOn)
    ...
    do
        ...
        tcHeaderFix.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        ...
        tcTitle.TextSize = 12
        tcTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
        ...
        local function makeTCRow(labelText, yPos, isOn, onToggle)
            ...
            -- TCRow Pill Hintergrund
            local tcPillBg = applyTextureBg(pill)
            if tcPillBg then tcPillBg.ZIndex = 0 end
        ...
        local function makeTCInputRow(labelText, yPos, currentVal, placeholder, onChange)
            ...
            -- TC Input Box Hintergrund
            local tcBoxBg = applyTextureBg(box)
            if tcBoxBg then tcBoxBg.ZIndex = 0 end
    ...
    updateUI = function(enabled, allPets)
        ...
        enableBtn.TextColor3 = enabled and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(170, 170, 170)
        if allPets then
        ...
        if SharedState.ListNeedsRedraw then
            ...
            if allPets and #allPets > 0 then
                for i = 1, #allPets do
                    ...
                    -- Row Hintergrund
                    local rowBg = applyTextureBg(row)
                    if rowBg then rowBg.ZIndex = 0 end
        ...
        for i, pb in ipairs(petButtons) do
            ...
            pb.button.BackgroundColor3 = sel and Color3.fromRGB(20, 20, 20) or Color3.fromRGB(30, 30, 30)
            pb.rank.TextColor3  = sel and Color3.fromRGB(245, 245, 245) or Color3.fromRGB(130, 130, 130)
            pb.info.TextColor3  = sel and Color3.fromRGB(250, 250, 250) or Color3.fromRGB(170, 170, 170)
        end
    ...
    local function createDuelBaseMarker(plot, sign)
        ...
        -- Duel Base Billboard Hintergrund
        local dbBg = applyTextureBg(bb)
        if dbBg then dbBg.ZIndex = 0 end
        ...
        -- Frame Hintergrund
        local frBg = applyTextureBg(frame)
        if frBg then frBg.ZIndex = 0 end
    ...
    local function makeLabel(adornee, offsetY)
        ...
        -- Base Timer Billboard Hintergrund
        local btBg = applyTextureBg(bb)
        if btBg then btBg.ZIndex = 0 end
...
do
    ...
    local function createStealBarGui()
        ...
        -- Steal Bar Container Hintergrund
        local sbBg = applyTextureBg(container)
        if sbBg then sbBg.ZIndex = 0 end
        ...
        -- Bar Background Hintergrund
        local bbBg = applyTextureBg(barBg)
        if bbBg then bbBg.ZIndex = 0 end
        ...
        -- Percent Box Hintergrund
        local pcBg = applyTextureBg(percentBox)
        if pcBg then pcBg.ZIndex = 0 end
...
frame.Size = UDim2.new(0, 480*mobileScale, 0, 480*mobileScale)
    ...
    -- Admin Panel Frame Hintergrund
    local apBg = applyTextureBg(frame)
    if apBg then apBg.ZIndex = 0 end
    ...
    -- Admin Header Hintergrund
    local ahBg = applyTextureBg(header)
    if ahBg then ahBg.ZIndex = 0 end
    ...
    -- Refresh Button Hintergrund
    local refBg2 = applyTextureBg(refreshBtn)
    if refBg2 then refBg2.ZIndex = 0 end
    ...
    proxCont.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    proxCont.BackgroundTransparency = 0.3
    Instance.new("UICorner", proxCont).CornerRadius = UDim.new(0, 10)

    -- Prox Container Hintergrund
    local pcBg = applyTextureBg(proxCont)
    if pcBg then pcBg.ZIndex = 0 end
    ...
    proxBtn.BackgroundColor3 = ProximityAPActive and Theme.Accent1 or Color3.fromRGB(50, 50, 50)
    proxBtn.Text = "Prox"
    ...
    -- Prox Button Hintergrund
    local pxBg = applyTextureBg(proxBtn)
    if pxBg then pxBg.ZIndex = 0 end

    local proxBtnStroke = Instance.new("UIStroke", proxBtn)
    proxBtnStroke.Color = ProximityAPActive and Theme.Accent2 or Color3.fromRGB(70, 70, 70)
    ...
    spamBaseBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
    ...
    -- Spam Base Button Hintergrund
    local sbBg = applyTextureBg(spamBaseBtn)
    if sbBg then sbBg.ZIndex = 0 end

    local spamBaseBtnStroke = Instance.new("UIStroke", spamBaseBtn)
    spamBaseBtnStroke.Color = Color3.fromRGB(70, 70, 70)
    ...
    -- Click AP Button Hintergrund
    local caBg = applyTextureBg(ctapPanelBtn)
    if caBg then caBg.ZIndex = 0 end

    local function updateCtapPanelBtn()
        ctapPanelBtn.BackgroundColor3 = Config.ClickToAP and Theme.Accent1 or Color3.fromRGB(50, 50, 50)
    ...
    ctapPanelStroke.Color = Color3.fromRGB(70, 70, 70)
    ...
    local function updateProximityAPButton()
        if SharedState.ProximityAPButton then
            SharedState.ProximityAPButton.BackgroundColor3 = ProximityAPActive and Theme.Accent1 or Color3.fromRGB(50, 50, 50)
            ...
            if SharedState.ProximityAPButtonStroke then
                SharedState.ProximityAPButtonStroke.Color = ProximityAPActive and Theme.Accent2 or Color3.fromRGB(70, 70, 70)
            end
        end
    end
    ...
    -- Tab Bar Hintergrund
    local tbBg = applyTextureBg(tabBar)
    if tbBg then tbBg.ZIndex = 0 end
    ...
    btn.Size = UDim2.new(0, 110, 0, 24)
        ...
        local tabBtnBg = applyTextureBg(btn)
        if tabBtnBg then tabBtnBg.ZIndex = 0 end
    ...
    local function setActiveTab(name)
        ...
        if name == "players" then
            ...
            tabBlacklistStroke.Color = Color3.fromRGB(100, 100, 100)
            ...
            tabPlayersStroke.Color = Color3.fromRGB(100, 100, 100)
    ...
    -- Blacklist Frame Hintergrund
    local blBg = applyTextureBg(blFrame)
    if blBg then blBg.ZIndex = 0 end
    ...
    -- Blacklist Input Hintergrund
    local blInBg = applyTextureBg(blInput)
    if blInBg then blInBg.ZIndex = 0 end

    local blInputStroke = Instance.new("UIStroke", blInput)
    blInputStroke.Color = Color3.fromRGB(80, 80, 80)
    ...
    -- Blacklist Add Button Hintergrund
    local blaBg = applyTextureBg(blAddBtn)
    if blaBg then blaBg.ZIndex = 0 end
    ...
    local function rebuildBlList()
        ...
        for i, name in ipairs(Config.BlacklistedUsers) do
            ...
            -- Blacklist Row Hintergrund
            local blRowBg = applyTextureBg(row)
            if blRowBg then blRowBg.ZIndex = 0 end
    ...
    emptyPlayersLbl.TextColor3 = Color3.fromRGB(150, 150, 150)
...
highlight.Adornee = newAdornee
    ...
    local function createPlayerRow(plr)
        ...
        row.Name = plr.Name
        row.LayoutOrder = 0
row.Size = UDim2.new(1, -4, 0, 74)        row.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        ...
        -- Player Row Hintergrund
        local prBg = applyTextureBg(row)
        if prBg then prBg.ZIndex = 0 end
    ...
    btnCont.BackgroundTransparency = 1; btnCont.ZIndex = 10
        ...
        for i, def in ipairs(buttonsDef) do
            ...
            -- Button Hintergrund
            local btnBg = applyTextureBg(b)
            if btnBg then btnBg.ZIndex = 0 end
            ...
            b.BackgroundColor3 = (def.cmd == "balloon" and hasBallooned) or isOnCD and Theme.Error or Color3.fromRGB(50, 50, 50)
            b.BackgroundTransparency = 0
            Instance.new("UICorner", b).CornerRadius = UDim.new(0, 8)
            local bStroke = Instance.new("UIStroke", b)
            bStroke.Color = (def.cmd == "balloon" and hasBallooned) or isOnCD and Theme.Error or Color3.fromRGB(100, 100, 100)
        ...
        -- Quick Blacklist Button Hintergrund
        local qbBg = applyTextureBg(blQuickBtn)
        if qbBg then qbBg.ZIndex = 0 end
...
ShowNotification("BLACKLIST", "Blacklisted: " .. targetName)
        ...
        -- TP Button Hintergrund
        local tpBg = applyTextureBg(tpBtn)
        if tpBg then tpBg.ZIndex = 0 end
...
local function showBigAlert(text)
    ...
    -- Big Alert Frame Hintergrund
    local baBg = applyTextureBg(f)
    if baBg then baBg.ZIndex = 0 end
...
if IS_MOBILE then
    ...
    -- Mobile Controls Frame Hintergrund
    local mcBg = applyTextureBg(controlsFrame)
    if mcBg then mcBg.ZIndex = 0 end
    ...
    local function createMobBtn(text, color, layoutOrder, callback)
        ...
        local mbBg2 = applyTextureBg(btn)
        if mbBg2 then mbBg2.ZIndex = 0 end
    ...
    createMobBtn("UI", Color3.fromRGB(255, 255, 255), 4, function()
    ...
    -- Reset Button Hintergrund
    local resBg = applyTextureBg(resetBtn)
    if resBg then resBg.ZIndex = 0 end
    ...
    -- Settings Button Hintergrund
    local setBg = applyTextureBg(openBtn)
    if setBg then setBg.ZIndex = 0 end
...
-- Settings Frame Hintergrund
local sfBg = applyTextureBg(sFrame)
if sfBg then sfBg.ZIndex = 0 end
...
-- Settings Header Hintergrund
local shBg = applyTextureBg(sHeader)
if shBg then shBg.ZIndex = 0 end
...
sTitle.BackgroundTransparency = 1; sTitle.Text = "Secret Hub Settings"
...
-- Tab Bar Hintergrund
local tbBg2 = applyTextureBg(tabBar)
if tbBg2 then tbBg2.ZIndex = 0 end
...
-- Tab Content Hintergrund
local tcBg = applyTextureBg(tabContent)
if tcBg then tcBg.ZIndex = 0 end
...
local function makeTabPage(key)
    ...
    -- Tab Page Hintergrund
    local tpBg = applyTextureBg(sf)
    if tpBg then tpBg.ZIndex = 0 end
...
for i, def in ipairs(TAB_DEFS) do
    ...
    -- Tab Button Hintergrund
    local tbBtnBg = applyTextureBg(b)
    if tbBtnBg then tbBtnBg.ZIndex = 0 end
...
local function CreateToggleSwitch(parent, initialState, callback)
    ...
    -- Toggle Switch Hintergrund
    local tsBg = applyTextureBg(sw)
    if tsBg then tsBg.ZIndex = 0 end
...
local function CreateSectionHeader(text)
    ...
    local shBg2 = applyTextureBg(accent)
    if shBg2 then shBg2.ZIndex = 0 end
...
do
    ...
    local function makeStepBtn(label, xPos)
        ...
        -- Step Button Hintergrund
        local stepBg = applyTextureBg(b)
        if stepBg then stepBg.ZIndex = 0 end

        local s = Instance.new("UIStroke", b)
        s.Color = Color3.fromRGB(80, 80, 80)
        s.Thickness = 1
        b.MouseEnter:Connect(function() b.BackgroundColor3 = Color3.fromRGB(65, 65, 65) end)
        b.MouseLeave:Connect(function() b.BackgroundColor3 = Color3.fromRGB(30, 30, 30) end)
        return b
    end
    ...
    valueLbl.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
    ...
    vStroke.Color = Color3.fromRGB(80, 80, 80)
    vStroke.Thickness = 1
...
-- Priority List Button Hintergrund
local plBg = applyTextureBg(bPriorityList)
if plBg then plBg.ZIndex = 0 end
...
do
    ...
    local function addTpPathSegment(a, b)
        ...
        halo.Color        = Color3.fromRGB(255, 25, 25)
...
-- TP Now Button Hintergrund
local tnBg = applyTextureBg(bTpNow)
if tnBg then tnBg.ZIndex = 0 end
...
toolSub.Text = "Carpet name used by all TP versions"
...
local toolBox = Instance.new("TextBox", rTpTool)
toolBox.Size = UDim2.new(0, 130, 0, 22); toolBox.Position = UDim2.new(1, -140, 0.5, -11)
toolBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30); toolBox.BorderSizePixel = 0
toolBox.Text = tostring(Config.TpSettings.Tool or "Flying Carpet")
toolBox.PlaceholderText = "Flying Carpet"
toolBox.Font = Enum.Font.Gotham; toolBox.TextSize = 11
toolBox.TextColor3 = Color3.fromRGB(255, 255, 255); toolBox.ClearTextOnFocus = false
Instance.new("UICorner", toolBox).CornerRadius = UDim.new(0, 4)

-- Tool Box Hintergrund
local tbBg3 = applyTextureBg(toolBox)
if tbBg3 then tbBg3.ZIndex = 0 end

local toolStroke = Instance.new("UIStroke", toolBox)
toolStroke.Color = Color3.fromRGB(80, 80, 80); toolStroke.Thickness = 1
toolBox.FocusLost:Connect(function()
    local raw = toolBox.Text:gsub("^%s+", ""):gsub("%s+$", "")
    if raw == "" then raw = "Flying Carpet" end
    Config.TpSettings.Tool = raw
    toolBox.Text = raw
    SaveConfig()
    ShowNotification("TP TOOL", raw)
end)
switchTab("Keys")
...
-- TP Key Button Hintergrund
local tkBg = applyTextureBg(bTpKey)
if tkBg then tkBg.ZIndex = 0 end
...
-- Clone Key Button Hintergrund
local ckBg = applyTextureBg(bCloneKey)
if ckBg then ckBg.ZIndex = 0 end
...
-- Tween Speed Button Hintergrund
local tsBg2 = applyTextureBg(bTweenSpeedOpen)
if tsBg2 then tsBg2.ZIndex = 0 end
...
local function _buildTweenSpeedGui()
    ...
    -- Tween Speed Panel Hintergrund
    local tspBg = applyTextureBg(panel)
    if tspBg then tspBg.ZIndex = 0 end
    ...
    -- Tween Speed Header Hintergrund
    local tshBg = applyTextureBg(header)
    if tshBg then tshBg.ZIndex = 0 end
    ...
    local function mkSpeedSlider(labelText, configKey, defaultVal, yPos)
        ...
        -- Speed Slider Row Hintergrund
        local ssrBg = applyTextureBg(row)
        if ssrBg then ssrBg.ZIndex = 0 end
...
-- Carpet Key Button Hintergrund
local ck2Bg = applyTextureBg(bCarpet)
if ck2Bg then ck2Bg.ZIndex = 0 end
...
-- Ragdoll Self Button Hintergrund
local rsBg = applyTextureBg(bRagdollSelf)
if rsBg then rsBg.ZIndex = 0 end
...
-- Steal Speed Key Button Hintergrund
local sskBg = applyTextureBg(bStealSpeedKey)
if sskBg then sskBg.ZIndex = 0 end
...
do
    ...
    autoSellPodiumBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    ...
    autoSellPodiumBox.TextColor3 = Color3.fromRGB(255, 255, 255)
    autoSellPodiumBox.ClearTextOnFocus = false
    Instance.new("UICorner", autoSellPodiumBox).CornerRadius = UDim.new(0, 4)

    -- Auto Sell Podium Box Hintergrund
    local aspBg = applyTextureBg(autoSellPodiumBox)
    if aspBg then aspBg.ZIndex = 0 end

    local autoSellPodiumStroke = Instance.new("UIStroke", autoSellPodiumBox)
    autoSellPodiumStroke.Color = Color3.fromRGB(80, 80, 80)
...
-- Reset Key Button Hintergrund
local rkBg = applyTextureBg(bResetKey)
if rkBg then rkBg.ZIndex = 0 end
...
-- Kick Key Button Hintergrund
local kkBg = applyTextureBg(bKickKey)
if kkBg then kkBg.ZIndex = 0 end
...
-- Auto Turret Key Button Hintergrund
local atkBg = applyTextureBg(bAutoTurretKey)
if atkBg then atkBg.ZIndex = 0 end
...
-- Click AP Key Button Hintergrund
local cakBg = applyTextureBg(bClickToAPKeybind)
if cakBg then cakBg.ZIndex = 0 end
...
-- Proximity AP Key Button Hintergrund
local pakBg = applyTextureBg(bProximityAPKeybind)
if pakBg then pakBg.ZIndex = 0 end
...
-- Sound Box Hintergrund
local sbBg4 = applyTextureBg(soundBox)
if sbBg4 then sbBg4.ZIndex = 0 end
...
-- Webhook Box Hintergrund
local whBg = applyTextureBg(webhookBox)
if whBg then whBg.ZIndex = 0 end
...
-- Joiner Key Button Hintergrund
local jkBg = applyTextureBg(bJoinerKey)
if jkBg then jkBg.ZIndex = 0 end
...
-- FOV Reset Button Hintergrund
local frBg = applyTextureBg(bFOVReset)
if frBg then frBg.ZIndex = 0 end
...
if not IS_MOBILE then
    ...
    -- Menu Button Hintergrund
    local muBg = applyTextureBg(bMenu)
    if muBg then muBg.ZIndex = 0 end
...
-- Reset Button Hintergrund
local resBg2 = applyTextureBg(bReset)
if resBg2 then resBg2.ZIndex = 0 end
...
task.spawn(function()
    ...
    -- Steal Speed Frame Hintergrund
    local ssfBg = applyTextureBg(ssFrame)
    if ssfBg then ssfBg.ZIndex = 0 end
    ...
    -- SS Button Hintergrund
    local ssbBg = applyTextureBg(ssBtn)
    if ssbBg then ssbBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    local function createBrainrotBillboard(data)
        ...
        -- Brainrot Billboard Hintergrund
        local brBg = applyTextureBg(bb)
        if brBg then brBg.ZIndex = 0 end
        ...
        local contBg = applyTextureBg(container)
        if contBg then contBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    local HTheme = {
        ...
        Accent1 = Color3.fromRGB(255, 70, 130),
        Accent2 = Color3.fromRGB(200, 30, 90),
        White   = Color3.fromRGB(255, 230, 240),
        Gray    = Color3.fromRGB(100,130,180),
    }
    local SCALE = 1
    local TOPBAR_H = 50 * SCALE
    local gui = Instance.new("ScreenGui")
    gui.Name = "SecretHubStatusHUD"
    gui.ResetOnSpawn = false
    ...
    -- Status HUD Main Hintergrund
    local shBg3 = applyTextureBg(main)
    if shBg3 then shBg3.ZIndex = 0 end
    ...
    -- Top Bar Hintergrund
    local tbBg4 = applyTextureBg(topBar)
    if tbBg4 then tbBg4.ZIndex = 0 end
    ...
    title.Text = privateBuild and "Secret Hub" or "Secret Hub"
    title.Font = Enum.Font.GothamBlack
    ...
    for i = 1, 3 do
        ...
        -- Unlock Button Hintergrund
        local ubBg = applyTextureBg(btn)
        if ubBg then ubBg.ZIndex = 0 end

        local btnStroke = Instance.new("UIStroke", btn)
        btnStroke.Color = Color3.fromRGB(200, 30, 90)
    ...
    subGui.Name = "SecretHubStatusHUDSub"
    ...
    serverPosGui.Name = "SecretHubServerPos"
    ...
    serverPosFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    ...
    -- Server Pos Frame Hintergrund
    local spBg = applyTextureBg(serverPosFrame)
    if spBg then spBg.ZIndex = 0 end

    local spStroke = Instance.new("UIStroke", serverPosFrame)
    spStroke.Color = Color3.fromRGB(80, 80, 80)
    ...
    spCaret.TextSize = 11
    spCaret.TextColor3 = Color3.fromRGB(180, 90, 125)
...
task.spawn(function()
    ...
    local function makePlayerBillboard(player)
        ...
        -- Player Billboard Hintergrund
        local pbBg = applyTextureBg(bb)
        if pbBg then pbBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    local function addStealBaseHighlight(plr)
        ...
        hl.Name = "SecretHubStealBaseHL_"..tostring(uid)
        ...
        local bb = Instance.new("BillboardGui")
        bb.Name = "SecretHubStealBaseBB_"..tostring(uid)
        bb.AlwaysOnTop = true
        bb.LightInfluence = 0
        bb.ResetOnSpawn = false
        ...
        -- Steal Base Billboard Hintergrund
        local sbbBg = applyTextureBg(bb)
        if sbbBg then sbbBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    local function createMineESP(mine)
        ...
        -- Mine Billboard Hintergrund
        local mbBg3 = applyTextureBg(billboardGui)
        if mbBg3 then mbBg3.ZIndex = 0 end
...
task.spawn(function()
    ...
    -- Priority List Main Frame Hintergrund
    local plmfBg = applyTextureBg(mainFrame)
    if plmfBg then plmfBg.ZIndex = 0 end
    ...
    local clBg = applyTextureBg(closeBtn)
    if clBg then clBg.ZIndex = 0 end
    ...
    local function refreshPriorityList()
        ...
        for i, petName in ipairs(PRIORITY_LIST) do
            ...
            -- Priority Item Frame Hintergrund
            local pifBg = applyTextureBg(itemFrame)
            if pifBg then pifBg.ZIndex = 0 end
            ...
            -- Up Button Hintergrund
            local upBg = applyTextureBg(upBtn)
            if upBg then upBg.ZIndex = 0 end
            ...
            -- Down Button Hintergrund
            local dnBg = applyTextureBg(downBtn)
            if dnBg then dnBg.ZIndex = 0 end
            ...
            -- Remove Button Hintergrund
            local rmBg = applyTextureBg(removeBtn)
            if rmBg then rmBg.ZIndex = 0 end
    ...
    local function refreshAvailableList()
        ...
        for _, petName in ipairs(secretPets) do
            ...
            -- Available Item Frame Hintergrund
            local aifBg = applyTextureBg(itemFrame)
            if aifBg then aifBg.ZIndex = 0 end
            ...
            -- Add Button Hintergrund
            local adBg = applyTextureBg(addBtn)
            if adBg then adBg.ZIndex = 0 end
    ...
    local saBg = applyTextureBg(saveBtn)
    if saBg then saBg.ZIndex = 0 end
    ...
    saveBtn.MouseButton1Click:Connect(function()
        ...
        successLabel.Text = "✔ Saved to file!"
        ...
        -- Success Label Hintergrund
        local slBg = applyTextureBg(successLabel)
        if slBg then slBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    -- Job Joiner Main Hintergrund
    local jjBg = applyTextureBg(main)
    if jjBg then jjBg.ZIndex = 0 end
    ...
    local function CreateInput(placeholder, width, default)
        ...
        -- Input Box Hintergrund
        local inBg = applyTextureBg(box)
        if inBg then inBg.ZIndex = 0 end
    ...
    local function CreateButton(text, width, color)
        ...
        -- Create Button Hintergrund
        local cbBg = applyTextureBg(btn)
        if cbBg then cbBg.ZIndex = 0 end
    ...
    -- Join Button Hintergrund
    local jbBg = applyTextureBg(joinBtn)
    if jbBg then jbBg.ZIndex = 0 end
    ...
    -- Clear Button Hintergrund
    local clrBg = applyTextureBg(clearBtn)
    if clrBg then clrBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    -- Auto Buy Panel Hintergrund
    local abpBg = applyTextureBg(abPanel)
    if abpBg then abpBg.ZIndex = 0 end
    ...
    local function makeAbRow(h, order)
        ...
        -- AB Row Hintergrund
        local abrBg = applyTextureBg(r)
        if abrBg then abrBg.ZIndex = 0 end
    ...
    -- Toggle Button Hintergrund
    local atbBg = applyTextureBg(abToggleBtn)
    if atbBg then atbBg.ZIndex = 0 end
    ...
    -- AB Key Button Hintergrund
    local akbBg = applyTextureBg(abKeyBtn)
    if akbBg then akbBg.ZIndex = 0 end
    ...
    local function buildCirclePresets()
        ...
        for i, col in ipairs(THEME_SWATCHES) do
            ...
            local colBg = applyTextureBg(cb)
            if colBg then colBg.ZIndex = 0 end
    ...
    -- Rejoin Button Hintergrund
    local rjBg = applyTextureBg(abRejoinBtn)
    if rjBg then rjBg.ZIndex = 0 end
...
do
    ...
    local function buildRemoteSell()
        ...
        screenGui_RS.Name = "SecretHubRemoteSell"
        ...
        local SAVE_FILE = "SecretHub_remotesell_pos.txt"
        ...
        -- Remote Sell Main Frame Hintergrund
        local rsmBg = applyTextureBg(mainFrame)
        if rsmBg then rsmBg.ZIndex = 0 end
        ...
        -- Title Bar Hintergrund
        local tbBg5 = applyTextureBg(titleBar)
        if tbBg5 then tbBg5.ZIndex = 0 end
        ...
        -- Scroll Frame Hintergrund
        local scBg = applyTextureBg(scrollFrame)
        if scBg then scBg.ZIndex = 0 end
        ...
        local function addRow(podiumNum, sellPrompt, sellText)
            ...
            -- Add Row Hintergrund
            local arBg = applyTextureBg(row)
            if arBg then arBg.ZIndex = 0 end
            ...
            -- Sell Button Hintergrund
            local seBg = applyTextureBg(sellBtn)
            if seBg then seBg.ZIndex = 0 end
            ...
            sellBtn.MouseButton1Click:Connect(function()
                ...
                if sellOk then
                    ...
                    sellBtn.Text = "ERR"
                    sellBtn.BackgroundColor3 = Color3.fromRGB(120, 25, 35)
                    task.wait(1)
                    sellBtn.Text = "SELL"
                    sellBtn.BackgroundColor3 = Theme.Accent2
                end
...
do
    ...
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj.Name == "SecretHubAutoSellZone" then
            pcall(function() obj:Destroy() end)
        end
    end
    ...
    local function buildVisual()
        ...
        part.Name           = "SecretHubAutoSellZone"
        part.Anchored       = true
        ...
        outline.Color3      = Color3.fromRGB(255, 40, 40)
        outline.Transparency = 0.1
        outline.SurfaceColor3 = Color3.fromRGB(255, 80, 80)
        ...
        -- Auto Sell Billboard Hintergrund
        local asbBg = applyTextureBg(bb)
        if asbBg then asbBg.ZIndex = 0 end
...
task.spawn(function()
    ...
    local function createErrorOrb()
        ...
        -- Error Orb Gui Hintergrund
        local eoBg = applyTextureBg(sg)
        if eoBg then eoBg.ZIndex = 0 end
    ...
    local function createServerGhost(position)
        ...
        -- Lagback Notification Gui Hintergrund
        local lnBg = applyTextureBg(sg)
        if lnBg then lnBg.ZIndex = 0 end
        ...
        -- Server Ghost Billboard Hintergrund
        local sgBg = applyTextureBg(bb)
        if sgBg then sgBg.ZIndex = 0 end
    ...
    -- Invis Panel Frame Hintergrund
    local ipBg = applyTextureBg(iFrame)
    if ipBg then ipBg.ZIndex = 0 end
    ...
    -- Invis Container Hintergrund
    local icBg = applyTextureBg(iContainer)
    if icBg then icBg.ZIndex = 0 end
    ...
    -- Invis Button Hintergrund
    local invBg = applyTextureBg(btnInvis)
    if invBg then invBg.ZIndex = 0 end
    ...
    -- Key Button Hintergrund
    local kbBg5 = applyTextureBg(keyBtn)
    if kbBg5 then kbBg5.ZIndex = 0 end
    ...
    -- Fix Button Hintergrund
    local fixBg = applyTextureBg(btnFix)
    if fixBg then fixBg.ZIndex = 0 end
    ...
    local function CreateFancySlider(parent, name, min, max, default, callback)
        ...
        if IS_MOBILE then
            ...
            -- Text Box Hintergrund
            local txBg = applyTextureBg(textBox)
            if txBg then txBg.ZIndex = 0 end
            ...
            fill.BackgroundColor3 = Color3.fromRGB(220, 50, 110)
            fill.ZIndex = 12
            fill.Parent = slideBg
            Instance.new("UICorner", fill).CornerRadius = UDim.new(1, 0)
...
task.spawn(function()
    ...
    if getgenv().SecretHub_GriefDetectorCleanup then
        pcall(getgenv().SecretHub_GriefDetectorCleanup)
    end
    local GD_Conns = {}
    ...
    local function GDCleanup()
        ...
        if getgenv().SecretHub_GriefDetectorUI then
            pcall(function() getgenv().SecretHub_GriefDetectorUI:Destroy() end)
        end
        getgenv().SecretHub_GriefDetectorCleanup = nil
    end
    getgenv().SecretHub_GriefDetectorCleanup = GDCleanup
    ...
    local ScreenGui = Instance.new("ScreenGui")
    getgenv().SecretHub_GriefDetectorUI = ScreenGui
    ScreenGui.Name = "SecretHub_GriefDetector"
    ScreenGui.ResetOnSpawn = false
    ScreenGui.Parent = PlayerGui_GD

    -- Grief Detector Gui Hintergrund
    local gdBg = applyTextureBg(ScreenGui)
    if gdBg then gdBg.ZIndex = 0 end
    ...
    local function Notify(Griefer, Victim, Item, Autojoiner, StealInfo)
        ...
        if #animalName > 20 then animalName = animalName:sub(1, 18) .. "..."
        end
        ...
        -- Card Hintergrund
        local cardBg = applyTextureBg(Card)
        if cardBg then cardBg.ZIndex = 0 end
        ...
        CardStroke.Thickness = 2; CardStroke.Transparency = 0.05; CardStroke.Color = Color3.fromRGB(255, 70, 130)
    ...
    local function SendWebhook(Entries, Victim, StealInfo)
        ...
        local embed = {
            ...
            color = 0xff4444, fields = {}, footer = { text = "Secret Hub" }
        }
        local desc = timeText .. "\n**Victim:** " .. Victim .. "\n"
